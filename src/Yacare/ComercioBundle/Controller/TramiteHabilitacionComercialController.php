...
class TramiteHabilitacionComercialController extends \Yacare\TramitesBundle\Controller\TramiteController {
	...
	public function consultarAction(Request $request) {
		...
		$editFormBuilder = $this->createFormBuilder ()->add ( 'Actividad1', 'Tapir\FormBundle\Form\Type\EntityIdType', array (
		...
		if ($porpartida) {
			$editFormBuilder->add ( 'Partida', 'Tapir\FormBundle\Form\Type\EntityIdType', array (
					'label' => 'Partida',
					'required' => true,
			...
			$editFormBuilder->add ( 'Local', 'Tapir\FormBundle\Form\Type\EntityIdType', array (
					'label' => 'Local',
		...
		if ($editForm->isValid ()) {
			...
			$res = $this->ConstruirResultado ( new \Tapir\AbmBundle\Helper\Resultados\ResultadoAsistenteAction ( $this ), $request );
			$res->usosuelo = $UsosSuelo;
			$res->porpartida = $porpartida;
			$res->comercio = $Comercio;
			$res->Tramite = $Tramite;
			$res->create = false;
			$res->Errores = '';
			$res->FormularioEditar = $editForm->createView ();
